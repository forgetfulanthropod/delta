...
**Owner flow**: Onboarding (role selection + full-screen before/after hero) → Design Studio (photo + prompt + AI variations with tweaks + *prominent* cost estimates + breakdowns surfaced directly on cards, make-current summary, confirm-before-send total, dedicated Cost Summary panel, and pipeline with full est. project cost — "ready to go" transparency in addition to/instead of just "Send to Sourcing"; for the Example Project the selected/approved design is shown first as hero, followed by reference photos & other concepts) → Sourcing (approve items, totals, retailer links) → Labor (realistic schedules with breaks and $25/hr costing).
