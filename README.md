...
- **AI**: Client provider/key UI (x/Gemini/OpenAI/Anthropic). Backend supports xAI (via env `XAI_API_KEY` or per-request key). Backend now leverages uploaded image via reference (data URI for web uploads or http) using xAI image edits endpoint for real visual understanding (not just text prompt); significantly richer image-aware prompts direct the model to analyze/ preserve exact room structure, perspective, lighting etc for realistic transformations. Image gen is image-reference-aware for better quality.
...
- AI generation now uses image references (when data URI or public URL provided from upload) + detailed visual analysis prompts for true image understanding and realistic remodel transformations (via xAI edits path); pure text fallback for non-data cases. (Still limited by model capabilities and demo data.)
